...
const Question = ({ 
  ...
  const speak = (text, autoNext = false) => {
    ...
    utterance.onend = () => {
      setIsPlaying(false);
      if (autoNext) {
        handleNext(); // Move to the next question/statement after feedback is spoken
      }
      ...
      if (currentView === 'question') {
        setShouldStartListening(true); // Activate the mic only for questions
      }
    };
  ...
  const handlePlay = () => {
    if (currentView === 'statement') {
      ...
      speak(`${isCorrect ? `${userAnswer} es correcto! ${feedback}` : `${userAnswer} es incorrecto! ${feedback}`}`, true); // Automatically go to next after feedback
    }
  };
  ...
  const handleMicrophoneClick = () => {
    if (listening) {
      ...
      // Automatically speak the answer feedback and move to next question/statement
      speak(`${isCorrect ? `${userAnswer} es correcto! ${feedback}` : `${userAnswer} es incorrecto! ${feedback}`}`, true);
    } else if (!isPlaying) {
      SpeechRecognition.startListening({ continuous: true, language: 'es-US' });
  ...
  const handleNext = () => {
    ...
    setCurrentView(showStatement ? 'statement' : 'question');
    setTranscriptLocked(false);
    // Automatically speak the new statement or question
    if (showStatement) {
      speak(statement);
    } else {
      speak(question);
  ...
  return (
    <>
      <div className="flex flex-col items-center pb-2 bg-gray-50 border-b min-h-80">
        {image && <img src={image} alt="question visual" className="h-72 rounded" />}
          ...
          <h2 className="text-lg font-semibold m-2 text-blue-700 max-w-72">
            ...
            {currentView === 'answer' && (
              <span className={isCorrect ? "text-green-500" : "text-red-500"}>
                {`${userAnswer} ${isCorrect ? 'es correcto! 11' : 'es incorrecto!'} ${feedback}`}
              </span>
            )}
          </h2>
      ...
      <div className='flex items-center justify-center mt-6'>
        ...
        <FontAwesomeIcon 
          ...
          className="cursor-pointer rounded-full text-blue-500 mr-6 text-4xl"
        />
        <button disabled={currentView !== 'answer'}>
          <FontAwesomeIcon 
            onClick={handleNext}
            icon={faCircleArrowRight} 
            className={`${currentView === 'answer' ? 'text-blue-500': 'text-gray-400'} text-4xl cursor-pointer`}
          />
        </button>
      </div>
