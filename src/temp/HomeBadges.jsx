...
const Home = () => {
  ...
  // Simulate word counts for each status (new, learning, review)
  const wordCounts = {
    new: 3,
    learning: 5,
    review: 2,
          ...
          <div key={story.id} className="bg-white rounded-lg shadow-lg mb-4 p-2 flex items-center space-x-3">
            ...
            <div className="flex flex-col justify-around items-between min-h-24">
              ...
              <div className="flex flex-wrap space-x-2 relative">
                {wordCounts.new > 0 && (
                  <span className="font-bold text-blue-500 rounded-full px-1 text-sm absolute top-0 left-0 transform -translate-y-4 translate-x-4">
                    {wordCounts.new}
                  </span>
                )}
                {wordCounts.learning > 0 && (
                  <span className="font-bold text-red-500 rounded-full px-1 text-sm absolute top-0 left-12 transform -translate-y-4 -translate-x-6">
                    {wordCounts.learning}
                  </span>
                )}
                {wordCounts.review > 0 && (
                  <span className="font-bold text-green-500 rounded-full px-1 text-sm absolute top-0 left-24 transform -translate-y-4 -translate-x-14">
                    {wordCounts.review}
                  </span>
                )}
